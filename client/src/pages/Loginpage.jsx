  ...
  return (
    ...
    gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
     ...
     {currentState === "Sign up" && !isDataSubmitted &&(
      <input type="text" placeholder='fullname' required className='p-2 border border-gray-500 
     ...
     {!isDataSubmitted && (
      ...
      type="password" placeholder='Password' required className='p-2 border
       border-gray-500 rounded-md bg-transparent focus:outline-none focus:ring-2
     ...
     <div className='flex items-center gap-2 text-sm text-gray-500'>
      <input type='checkbox'/>
      <p>Agree to terms of use and privasy policy.</p>
     </div>
     <div className='flex flex-col gap-2'>
      {currentState === "Sign up"?(
        <p className='text-sm text-gray-600'> Already have an account ?
        <span className='font-mediul text-violet-500 cursor-pointer' onClick={()=>SetCurrentState("Login")}>Login Here</span></p>
      ):(
        <p className='text-sm text-gray-600'>
          Create an account<span className='font-mediul text-violet-500 cursor-pointer ' onClick={()=>SetCurrentState("Sign up")} >Click here</span>
        </p>
      )}
     </div>
